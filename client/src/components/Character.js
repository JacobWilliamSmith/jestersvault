...
export default function Character(props) {
      ...
      <Grid key={columnIndex} item xs={columnLayout.width}>
        <Stack direction="row" sx={{ mt: 0.25 }} alignItems="flex-end" spacing={1}>
          ...
          <TextField
            ...
            InputProps={
              !columnLayout.hasStartAdornment
                ? {}
                : {
                    startAdornment: (
  ...
  function CompressedView() {
    return (
      <Stack
        className={props.id === activeCharacterId ? "activeCharacter" : null}
        direction="row"
        alignItems="bottom"
        spacing={1}
        sx={{ pl: 1, pr: 1, pt: 0.25, pb: 0.25 }}
      >
        <Grid container spacing={1}>
          {tableLayout.map((column) =>
            CompressedViewCell(tableLayout.findIndex((i) => i.stat === column.stat))
          )}
        </Grid>
      </Stack>
    );
  ...
  function ExpandedView() {
    return (
      <Card
        raised={props.id === activeCharacterId}
        sx={{ display: "flex", mt: 1, mb: 1, ml: 7, mr: 7 }}
      >
        <Grid
          className={props.id === activeCharacterId ? "activeCharacter" : null}
          style={
            props.id === activeCharacterId
              ? { borderRadius: "5px", borderStyle: "solid", borderWidth: "thin" }
              : {}
          }
          container
          spacing={0}
        >
          <Grid item xs={2}>
            <CardMedia
              ...
              image={
                character.image !== null && character.image !== undefined && character.image !== ""
                  ? character.image
  ...
  return (
    <div>
      {isCharacterExpanded ? ExpandedView() : CompressedView()}
      <MenuTextfield
        anchor={menuAnchor}
