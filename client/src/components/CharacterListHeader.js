    ...
    return (
      <Grid key={key} item xs={tableLayout[index].width}>
        <Stack direction="row" alignItems="flex-end" spacing={1} sx={{ pl: isLeftmost ? 6 : 0 }}>
          <h3 className="headerTitle">{tableLayout[index].name}</h3>
          <IconButton
            size="small"
            onClick={() => {
              handleOrder(tableLayout[index].stat);
            }}
          >
            {characterOrder?.orderBy !== tableLayout[index].stat ? (
              <UnorderedIcon fontSize="small" />
            ) : characterOrder?.isAscending ? (
              <OrderedAscendingIcon fontSize="small" />
            ) : (
              <OrderedDescendingIcon fontSize="small" />
            )}
          </IconButton>
        ...
        <Stack direction="row" alignItems="bottom" spacing={1} sx={{ ml: 1, mr: 1 }}>
          <Grid container spacing={1}>
            {tableLayout.map((column) =>
              CustomHeader(
                column.stat,
                tableLayout.findIndex((i) => i.stat === column.stat)
              )
            )}
          </Grid>
        </Stack>
