...
export default function PresetMenu(props) {
  ...
  const { characterPresets, setCharacterPresets, gamePresets, setGamePresets } =
  ...
  useEffect(() => {
    setEntries(
      (tab === 0 ? characterPresets : gamePresets).sort((p1, p2) => (p1.name > p2.name ? 1 : -1))
    );
          ...
          <Box key={preset._id}>
            <Stack direction="row" sx={{ pr: 1, width: "100%" }} alignItems="center" spacing={1}>
              <MenuItem
                ...
                sx={{ width: "100%" }}
              >
                <Typography noWrap>{preset.name}</Typography>
              </MenuItem>
